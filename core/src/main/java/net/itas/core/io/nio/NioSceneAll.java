 ...
 * @author liuzhen
 */
public final class NioSceneAll {
	...
	/**城市中玩家*/
	private final Map<String, NioScene> scenes;

	public NioSceneAll(int size) {
		this.user_group = new HashMap<>(size);
		this.scenes = new HashMap<String, NioScene>();
	 ...
	 * @return String 域
	 */
	public NioScene getScene(String sceneId)  {
		synchronized (scenes)  {
			NioScene scene = scenes.get(sceneId);
			
			if (scene == null)  {
				scenes.put(sceneId, scene = new NioScene(sceneId));
			}

...
public abstract class Message implements Rule {
	...
	protected abstract Message parse(ByteBuf buf);
	...
	protected void writeMessage(ByteBuf ByteBuf, Message msg) {
	...
	protected <T extends Message> T readMessage(Class<?> type, ByteBuf ByteBuf) {
		return (T) newBuilder(type).parse(ByteBuf);
	}
	...
	protected <T> List<T> readArray(ByteBuf buf, Class<?> classType) {
		...
		for(int i = 0; i < len; i ++) {
			...
			} else if (Message.class.isAssignableFrom(classType)) {
	...
	@SuppressWarnings("unchecked")
	private <T extends Message> T newBuilder(Class<?> classType) {
		try {
			Message msg;

package net.itas.core.bytecode;

import static net.itas.core.util.Utils.ByteCodeUtils.getTableName;
import static net.itas.core.util.Utils.ByteCodeUtils.name;
import static net.itas.core.util.Utils.ByteCodeUtils.uname;
import static net.itas.core.util.Utils.ByteCodeUtils.unameBool;
import static net.itas.core.util.Utils.CtClassUtils.getAllField;
import static net.itas.core.util.Utils.CtClassUtils.isBasicType;
import static net.itas.core.util.Utils.CtClassUtils.isBasicWarpType;
import static net.itas.core.util.Utils.CtClassUtils.isExtends;
...
import net.itas.core.GameBaseAotuID;
...
import net.itas.core.util.Utils.CtClassUtils;
import net.itas.util.GenericInfo;
...
//		clazz.writeFile("D:/");
	...
	private String getByte(CtField field)  {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("set%s(result.getByte(\"%s\"));", uname(field), name(field)));
		
		return buffer.toString();
	}
	
	private String setByte(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("state.setByte(%s, get%s());", index, uname(field)));
		
		return buffer.toString();
	}
	
	private String getBoolean(CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("set%s(result.getBoolean(\"%s\"));", unameBool(field), name(field)));
		
		return buffer.toString();
	}
	
	private String setBoolean(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("state.setBoolean(%s, is%s());", index, unameBool(field)));
		
		return buffer.toString();
	}
	
	private String getChar(CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("set%s(result.getString(\"%s\").charAt(0));", uname(field), name(field)));
		
		return buffer.toString();
	}
	
	private String setChar(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("state.setString(%s, String.valueOf(get%s()));", index, uname(field)));
		
		return buffer.toString();
	}
	

	private String getShort(CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("set%s(result.getShort(\"%s\"));", uname(field), name(field)));
		
		return buffer.toString();
	}
	
	private String setShort(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("state.setShort(%s, get%s());", index, uname(field)));
		
		return buffer.toString();
	}
	
	private String getInt(CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("set%s(result.getInt(\"%s\"));", uname(field), name(field)));
		
		return buffer.toString();
	}
	
	private String setInt(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
	...
	private String getLong(CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("set%s(result.getLong(\"%s\"));", uname(field), name(field)));
		
		return buffer.toString();
	}
	
	private String setLong(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
	...
	private String getFloat(CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("set%s(result.getFloat(\"%s\"));", uname(field), name(field)));
		
		return buffer.toString();
	}
	
	private String setFloat(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("state.setFloat(%s, get%s());", index, uname(field)));
		
		return buffer.toString();
	}
	
	private String getDouble(CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("set%s(result.getDouble(\"%s\"));", uname(field), name(field)));
		
		return buffer.toString();
	}
	
	private String setDouble(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("state.setDouble(%s, get%s());", index, uname(field)));
		
		return buffer.toString();
	}
	
	
	private String getString(CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("set%s(result.getString(\"%s\"));", uname(field), name(field)));
		
		return buffer.toString();
	}
	
	private String setString(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
	...
	private String getTimestamp(CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("set%s(result.getTimestamp(\"%s\"));", uname(field), name(field)));
		
		return buffer.toString();
	}
	
	private String setTimestamp(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("state.setTimestamp(%s, get%s());", index, uname(field)));
		
		return buffer.toString();
	}
	
	private String getSimple(CtField field, ClassPool pool) throws Exception {
		GenericInfo info = GenericUtils.getGenericInfo(field.getGenericSignature(), true);
		CtClass genericType = pool.get(info.getChilds().get(0).getName());
		...
		if (CtClassUtils.isExtends(genericType, GameObject.class)) {
			buffer.append(String.format("int dataId = result.getInt(\"%s\");", name(field)));
			buffer.append(nextRow(3));
			buffer.append("if (dataId > 0) {");
			buffer.append(nextRow(4));
			buffer.append(String.format("set%s(new net.itas.core.Simple(%s.class, dataId));", uname(field), genericType.getName()));
			buffer.append(nextRow(3));
			buffer.append("}");
		} else {
			buffer.append(String.format("java.lang.String dataId = result.getString(\"%s\");", name(field)));
			buffer.append(nextRow(3));
			buffer.append("if (dataId != null && !dataId.equals(\"\")) {");
			buffer.append(nextRow(4));
			buffer.append(String.format("set%s(new net.itas.core.Simple(%s.class, dataId));", uname(field), genericType.getName()));
			buffer.append(nextRow(3));
			buffer.append("}");
	...
	private String setSimple(int index, CtField field, ClassPool pool) throws Exception {
		GenericInfo info = GenericUtils.getGenericInfo(field.getGenericSignature(), true);
		CtClass genericType = pool.get(info.getChilds().get(0).getName());
		...
		buffer.append(nextRow(3));
		if (CtClassUtils.isExtends(genericType, GameObject.class)) {
			buffer.append("java.lang.Integer dataId = new java.lang.Integer(0);");
			buffer.append(nextRow(3));
			buffer.append(String.format("if(get%s() != null) {", uname(field)));
			buffer.append(nextRow(4));
			buffer.append(String.format("dataId = (java.lang.Integer)get%s().getId();", uname(field)));
			buffer.append(nextRow(3));
			buffer.append("}");
			buffer.append(nextRow(3));
			buffer.append(String.format("state.setInt(%s, dataId.intValue());", index));
		} else {
			buffer.append("java.lang.Object dataId = null;");
			buffer.append(nextRow(3));
			buffer.append(String.format("if(get%s() != null) {", uname(field)));
			buffer.append(nextRow(4));
			buffer.append(String.format("dataId = get%s().getId();", uname(field)));
			buffer.append(nextRow(3));
			buffer.append("}");
			buffer.append(nextRow(3));
			buffer.append(String.format("state.setString(%s, (java.lang.String)dataId);", index));
		}
		buffer.append(nextRow(2));
	...
	private String getSource(CtField field) throws NotFoundException {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("set%s((%s)net.itas.core.Pool.getResource(result.getString(\"%s\")));", uname(field), field.getType().getName(), name(field)));
		
		return buffer.toString();
	}

	private String setSource(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append("{");
		buffer.append(nextRow(3));
		buffer.append("String rid = null;");
		buffer.append(nextRow(3));
		buffer.append(String.format("if(get%s() != null) {", uname(field)));
		buffer.append(nextRow(4));
		buffer.append(String.format("rid = get%s().getId();", uname(field)));
		buffer.append(nextRow(3));
		buffer.append("}");
		buffer.append(nextRow(3));
		buffer.append(String.format("state.setString(%s, rid);", index));
		buffer.append(nextRow(2));
		buffer.append("}");
	...
	private String getCollection(CtField field, ClassPool pool) throws Exception {
		GenericInfo info = GenericUtils.getGenericInfo(field.getGenericSignature(), true);
		CtClass genericType = pool.get(info.getChilds().get(0).getName());
		String childChildGeneric = null;
		if (!info.getChilds().get(0).getChilds().isEmpty()) {
			childChildGeneric = info.getChilds().get(0).getChilds().get(0).getName().replace("$", ".");
		}
		...
		buffer.append(nextRow(3));
		buffer.append("java.util.List dataList = parserList(datas);");
		buffer.append(nextRow(3));
		buffer.append(String.format("%s dataArray = %s", info.getName(), newInstanceCollection(field)));
		buffer.append(nextRow(3));
		buffer.append("for (int i = 0; i < dataList.size(); i ++) {");
		buffer.append(nextRow(4));
		buffer.append(String.format("dataArray.add(%s);", getGenericStr(genericType, childChildGeneric, "dataList.get(i)")));
		buffer.append(nextRow(3));
		buffer.append("}");
		buffer.append(nextRow(3));
	...
	private String setCollection(int index, CtField field) {
		StringBuffer buffer = new StringBuffer();
		
		buffer.append(nextRow(2));
		buffer.append(String.format("state.setString(%s, getCollectionValue(get%s()));", index, uname(field)));
		
		return buffer.toString();
	}
	
	private String newInstanceCollection(CtField field) throws Exception {
		...
		if (isExtends(field, List.class)) {
			return "new java.util.ArrayList(8);";
		}
		
		if (isExtends(field, Set.class)) {
			return "new java.util.HashSet(8);";
		}

  ...
  @SuppressWarnings({ "unchecked", "rawtypes" })
	void fill(Field field, String text) throws Exception {
		...
		if (booleanType.isType(field.getType())) {
		  ...
		  field.set(this, parse((Class<Enum>)field.getType(), parseByte(text)));
		} else if (enumIntType.isType(field.getType())) {
		  field.set(this, parse((Class<Enum>)field.getType(), parseInt(text)));
		} else if (enumStringType.isType(field.getType())) {
	      field.set(this, parse((Class<Enum>)field.getType(), text));
